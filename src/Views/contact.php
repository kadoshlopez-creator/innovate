    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid lg:grid-cols-5 gap-10">
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <?php
                $infoItems = [
                    ['fas fa-envelope',       '#38bdf8', 'bg-sky-500/10 border-sky-500/15',     __('contact.info.email', 'Email'),     $contactEmail, "mailto:{$contactEmail}"],
                    ['fas fa-phone',          '#a78bfa', 'bg-violet-500/10 border-violet-500/15',__('contact.info.phone', 'Teléfono'),  $contactPhone, 'tel:' . preg_replace('/\s/', '', $contactPhone)],
                    ['fas fa-map-marker-alt', '#34d399', 'bg-green-500/10 border-green-500/15',  __('contact.info.location', 'Ubicación'), $contactAddr,  null],
                ];
                ...
                <div class="glass border border-white/[0.07] rounded-2xl p-6 mt-2">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <span class="text-green-400 text-sm font-semibold"><?= __('contact.status.available', 'Disponibles ahora') ?></span>
                    </div>
                    <p class="text-slate-400 text-sm leading-relaxed">
                        <?= __('contact.status.response_time', 'Tiempo de respuesta promedio: <span class="text-white font-semibold">&lt; 24 horas</span>. Para urgencias, escríbenos directamente por teléfono o WhatsApp.') ?>
                    </p>
                </div>
            </div>
            ...
            <div class="lg:col-span-3">
                <div class="glass border border-white/[0.07] rounded-3xl p-8 sm:p-10">
                    <h2 class="text-xl font-bold text-white mb-7"><?= __('contact.form.title', 'Cuéntanos sobre tu proyecto') ?></h2>
                        ...
                        <div class="grid sm:grid-cols-2 gap-5">
                            <div>
                                <label class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">
                                    <?= __('contact.form.name', 'Nombre') ?> <span class="text-red-400 normal-case text-sm">*</span>
                                </label>
                                <input type="text" name="name" value="<?= old('name') ?>" required
                                       ...
                                       placeholder="<?= __('contact.form.name_placeholder', 'Tu nombre completo') ?>">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">
                                    <?= __('contact.form.email', 'Email') ?> <span class="text-red-400 normal-case text-sm">*</span>
                                </label>
                                <input type="email" name="email" value="<?= old('email') ?>" required
                                       class="w-full bg-white/[0.04] border border-white/[0.08] focus:border-sky-500/50 focus:bg-sky-500/[0.03] rounded-xl px-4 py-3 text-white placeholder-slate-600 text-sm outline-none transition-all"
                                       placeholder="<?= __('contact.form.email_placeholder', 'tu@email.com') ?>">
                            </div>
                        </div>

                        <div class="grid sm:grid-cols-2 gap-5">
                            <div>
                                <label class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2"><?= __('contact.form.phone', 'Teléfono') ?></label>
                                <input type="tel" name="phone" value="<?= old('phone') ?>"
                                       class="w-full bg-white/[0.04] border border-white/[0.08] focus:border-sky-500/50 focus:bg-sky-500/[0.03] rounded-xl px-4 py-3 text-white placeholder-slate-600 text-sm outline-none transition-all"
                                       placeholder="<?= __('contact.form.phone_placeholder', '+507 0000-0000') ?>">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2"><?= __('contact.form.company', 'Empresa') ?></label>
                                <input type="text" name="company" value="<?= old('company') ?>"
                                       class="w-full bg-white/[0.04] border border-white/[0.08] focus:border-sky-500/50 focus:bg-sky-500/[0.03] rounded-xl px-4 py-3 text-white placeholder-slate-600 text-sm outline-none transition-all"
                                       placeholder="<?= __('contact.form.company_placeholder', 'Tu empresa (opcional)') ?>">
                            </div>
                        </div>

                        <div>
                            <label class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2"><?= __('contact.form.service', 'Servicio de interés') ?></label>
                            <select name="service"
                                ...
                                <option value=""><?= __('contact.form.service_placeholder', '— Selecciona un servicio —') ?></option>
                                ...
                                <?php endforeach; ?>
                                <option value="Otro"><?= __('contact.form.service_other', 'Otro') ?></option>
                            </select>
                        </div>

                        <div>
                            <label class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">
                                <?= __('contact.form.message', 'Mensaje') ?> <span class="text-red-400 normal-case text-sm">*</span>
                            </label>
                            <textarea name="message" required rows="5"
                                      ...
                                      placeholder="<?= __('contact.form.message_placeholder', 'Describe tu proyecto, objetivos o preguntas...') ?>"><?= old('message') ?></textarea>
                        </div>

                        <button type="submit"
                                class="btn-neon w-full text-white font-bold py-4 rounded-2xl text-sm flex items-center justify-center gap-2.5">
                            <i class="fas fa-paper-plane"></i> <?= __('contact.form.submit', 'Enviar Mensaje') ?>
                        </button>

                        <p class="text-center text-slate-600 text-xs">
                            <?= __('contact.form.privacy', 'Al enviar, aceptas que nos pongamos en contacto contigo. Sin spam.') ?>
                        </p>
                    </form>
                </div>
            </div>
